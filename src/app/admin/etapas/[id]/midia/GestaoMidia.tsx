...
export function GestaoMidia({
  ...
  const formFotos = useRef<HTMLFormElement>(null)
  ...
  async function acaoFotos(formData: FormData) {
    setEnviando(true)
    await enviarFotos(formData)
    setEnviando(false)
    formFotos.current?.reset()
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="flex flex-col gap-6">
        ...
        <section className="bg-surface border border-border rounded-2xl p-6">
          ...
          <form ref={formFotos} action={acaoFotos} className="flex flex-col gap-3">
            <input type="hidden" name="etapa_id" value={etapaId} />
            <input
              name="fotos"
              type="file"
            ...
            <p className="text-white/30 text-[11px]">
              Pode selecionar várias de uma vez. O envio pode demorar alguns segundos por foto.
            </p>
            <button
              type="submit"
              disabled={enviando}
      ...
      <div className="flex flex-col gap-6">
        <section className="bg-surface border border-border rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            {videos.length === 0 && (
              <p className="text-white/40 text-sm">Nenhum vídeo ainda.</p>
            )}
          </div>
        </section>
          ...
          {fotos.length === 0 ? (
            ...
            <div className="grid grid-cols-3 gap-2">
              {fotos.map((f) => (
                <div
                  key={f.id}
                  className="relative aspect-square overflow-hidden rounded-xl border border-border bg-bg"
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={f.url} alt="" className="h-full w-full object-cover" />
                  <form
                    action={excluirMidia.bind(null, f.id)}
                    className="absolute top-1 right-1"
                  >
                    <button
                      type="submit"
                      title="Remover"
                      className="rounded-lg bg-black/70 p-1.5 text-white/70 hover:text-red-400 transition-colors"
                    >
                      <Trash2 size={14} />
                    </button>
                  </form>
